...
<script lang="ts">
    ...
    import { update } from '@/routes/activities';
    ...
    const form = useForm({
        ...
        renja_id: (activity.renja_id === null ? '' : activity.renja_id) as any,
    ...
    function handleSubmit(e: Event) {
        ...
        form.put(toUrl(update({ activity: activity.id })));
    }
</script>

<AppHead title={`Edit Kegiatan - ${activity.name}`} />

<div class="p-6 space-y-6 w-full">
    <PageHeader
        title={`Edit Kegiatan: ${activity.name}`}
        description="Perbarui kode kegiatan, unit pelaksana, status, dan progress kegiatan"
    >
        {#snippet actions()}
            <Link
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <input
                            ...
                            bind:value={form.progress_percentage}
                            class="w-full px-3 py-2 text-sm bg-background border border-zinc-200 dark:border-zinc-800 rounded-lg outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all"
                            min="0"
                            max="100"
                            required
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {#each form.indicators as ind, index}
                            ...
                            >
                                ...
                                <div class="grid gap-3">
                                    <!-- Keep track of indicator ID for syncing if it exists -->
                                    {#if ind.id}
                                        <input
                                            type="hidden"
                                            bind:value={ind.id}
