...
<script lang="ts">
    ...
    import { update } from '@/routes/renja';
    ...
    const form = useForm({
        title: renja.title,
        fiscal_year_id: renja.fiscal_year_id,
        renstra_id: (renja.renstra_id === null ? '' : renja.renstra_id) as any,
        unit_id: renja.unit_id,
        status: renja.status,
        total_budget: renja.total_budget,
    });

    function handleSubmit(e: Event) {
        e.preventDefault();
        form.renstra_id = form.renstra_id === '' ? null : form.renstra_id;
        form.put(toUrl(update({ renja: renja.id })));
    }
</script>

<AppHead title={`Edit Renja - ${renja.title}`} />

<div class="p-6 space-y-6 w-full">
    <PageHeader
        title={`Edit Renja: ${renja.title}`}
        description="Perbarui program kerja tahunan pelaksana"
    >
        {#snippet actions()}
            <Link
