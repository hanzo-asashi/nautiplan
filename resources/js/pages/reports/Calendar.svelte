...
<script lang="ts">
    ...
    let {
        ...
        filters: { fiscal_year_id: number | null; unit_id: number | null; status: string | null };
    } = $props();

    // Get active/selected year
    const activeYear = $derived(
        fiscalYears.find((f) => f.id === filters.fiscal_year_id)?.year || new Date().getFullYear()
    );
...
firstDayIndex = 6;
        ...
        const prevMonthTotalDays = new Date(activeYear, currentMonth, 0).getDate();

        for (let i = firstDayIndex - 1; i >= 0; i--) {
            const dayNum = prevMonthTotalDays - i;
            const date = new Date(currentMonth === 0 ? activeYear - 1 : activeYear, currentMonth === 0 ? 11 : currentMonth - 1, dayNum);
        ...
        for (let i = 1; i <= nextMonthPadding; i++) {
            const date = new Date(currentMonth === 11 ? activeYear + 1 : activeYear, currentMonth === 11 ? 0 : currentMonth + 1, i);
    ...
    let selectedDate = $state<Date>(new Date(activeYear, currentMonth, new Date().getDate()));
...
<div class="flex h-full flex-1 flex-col gap-6 p-6 overflow-x-hidden">
    ...
    <!-- Filter Toolbar -->
    <div class="flex flex-wrap gap-4 items-center justify-between p-4 bg-card/40 border border-sidebar-border/30 rounded-xl backdrop-blur-md">
        <div class="flex flex-wrap gap-3 items-center">
            <!-- Year -->
            <div class="flex items-center gap-2">
                <span class="text-xs text-muted-foreground font-semibold">Tahun:</span>
                <select
                    value={filters.fiscal_year_id}
                    onchange={(e) => applyFilters({ fiscal_year_id: Number((e.target as HTMLSelectElement).value) })}
            ...
            <!-- Unit -->
            <div class="flex items-center gap-2">
                <span class="text-xs text-muted-foreground font-semibold">Unit:</span>
                <select
                    value={filters.unit_id || ''}
                    onchange={(e) => applyFilters({ unit_id: (e.target as HTMLSelectElement).value ? Number((e.target as HTMLSelectElement).value) : null })}
            ...
            <div class="flex items-center gap-2">
                ...
                <select
                    ...
                    onchange={(e) => applyFilters({ status: (e.target as HTMLSelectElement).value || null })}
                    class="px-2.5 py-1.5 text-xs bg-background border border-zinc-200 dark:border-zinc-800 rounded-md font-semibold cursor-pointer outline-none"
                >
        ...
        <div class="flex items-center gap-3">
            ...
            <span class="text-xs font-bold text-foreground min-w-[120px] text-center uppercase tracking-wide">
                {monthNames[currentMonth]} {activeYear}
            </span>
            <button
                onclick={nextMonth}
                class="p-1.5 border border-zinc-200 dark:border-zinc-800 bg-background/50 hover:bg-zinc-100 rounded-md transition-colors cursor-pointer"
    ...
    <div class="grid gap-6 lg:grid-cols-3 items-start flex-1">
        ...
        <div class="lg:col-span-2 rounded-xl border border-sidebar-border/30 bg-card/40 shadow-sm overflow-hidden backdrop-blur-md">
            <!-- Day names -->
            <div class="grid grid-cols-7 border-b border-sidebar-border/30 bg-zinc-100/50 dark:bg-zinc-900/50 text-center py-2 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
            ...
            <!-- Days cells -->
            <div class="grid grid-cols-7 divide-x divide-y divide-sidebar-border/20">
                {#each calendarDays as day}
                    ...
                    {@const isToday = new Date().getDate() === day.date.getDate() && new Date().getMonth() === day.date.getMonth() && new Date().getFullYear() === day.date.getFullYear()}
                    ...
                    <button
                        ...
                        class="h-24 p-2 text-left relative flex flex-col justify-between hover:bg-zinc-100/30 dark:hover:bg-zinc-900/20 transition-all cursor-pointer
                            {day.isCurrentMonth ? 'text-foreground' : 'text-muted-foreground/40 bg-zinc-50/20 dark:bg-zinc-950/5'}
                            {isSelected ? 'bg-primary/5 ring-1 ring-primary' : ''}"
                    >
                        <!-- Day Number -->
                        <span class="text-xs font-bold rounded-full size-6 flex items-center justify-center
                            {isToday ? 'bg-primary text-white shadow-md' : 'text-foreground/80'}">
                            {day.dayNumber}
                        </span>
                        ...
                        <div class="space-y-1 w-full overflow-hidden">
                            {#each dayEvents.slice(0, 3) as event}
                                <div class="h-1.5 w-full rounded-full truncate text-[8px] px-1 font-bold flex items-center justify-center leading-none text-white
                                    {event.type === 'activity' ? 'bg-primary' : 'bg-amber-500'}">
                                    <span class="hidden sm:inline truncate">{event.type === 'activity' ? event.code : event.name}</span>
                                </div>
                            {/each}
                            {#if dayEvents.length > 3}
                                <div class="text-[8px] text-muted-foreground text-center font-bold">
        ...
        <div class="rounded-xl border border-sidebar-border/30 bg-card/40 p-5 shadow-sm backdrop-blur-md space-y-4">
            <div class="flex items-center gap-2 border-b border-sidebar-border/20 pb-3">
            ...
            <div class="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {#if selectedDayEvents.length === 0}
                    <div class="p-8 text-center text-xs text-muted-foreground/60 italic space-y-2">
                        ...
                        <p>Tidak ada kegiatan atau sub-kegiatan yang dijadwalkan pada hari ini.</p>
                    </div>
                {:else}
                    {#each selectedDayEvents as event}
                        <div class="p-3 bg-background border border-sidebar-border/20 rounded-xl space-y-2 hover:border-sidebar-border/30 transition-all shadow-sm">
                            <div class="flex justify-between items-start gap-2">
                                <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider
                                    {event.type === 'activity' ? 'bg-primary/10 text-primary border border-primary/20' : 'bg-amber-500/10 text-amber-600 border border-amber-500/20'}">
                                    {event.type === 'activity' ? 'Kegiatan' : 'Sub-Kegiatan'}
                                </span>
                                <span class="text-[9px] font-bold text-primary">{event.progress}%</span>
                            </div>

                            <h4 class="text-xs font-bold text-foreground">
                                {#if event.code}
                                    <span class="text-muted-foreground font-semibold mr-1">[{event.code}]</span>
                                {/if}
                                {event.name}
                            </h4>

                            <div class="text-[10px] text-muted-foreground space-y-1 leading-relaxed">
                                <p><span class="font-semibold text-foreground/75">Unit Kerja:</span> {event.unit}</p>
                                <p><span class="font-semibold text-foreground/75">Mulai:</span> {event.start.toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</p>
                                <p><span class="font-semibold text-foreground/75">Selesai:</span> {event.end.toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</p>
                                {#if event.assignee}
                                    <p><span class="font-semibold text-foreground/75">PJ Pelaksana:</span> {event.assignee}</p>
                                {/if}
                            </div>
                        </div>
                    {/each}
