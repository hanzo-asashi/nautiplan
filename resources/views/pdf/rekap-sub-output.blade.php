...
<head>
    <meta charset="UTF-8">
    <title>Laporan Realisasi Per Sub Output</title>
    <style>
        body {
...
<body>
    ...
    <div class="title-block">
        <div class="title">LAPORAN REALISASI ANGGARAN PER SUB OUTPUT</div>
    ...
    <table class="border-table">
        <thead>
            <tr>
                ...
                <th style="width: 15%;">Kode Sub Output</th>
                <th>Nama Sub Output</th>
                <th style="width: 25%; text-align: right;">Pagu DIPA (Rp)</th>
                <th style="width: 25%; text-align: right;">Realisasi Belanja (Rp)</th>
                <th style="width: 15%; text-align: center;">Persentase (%)</th>
            </tr>
        </thead>
        <tbody>
            @foreach($subOutputs as $index => $so)
                <tr>
                    <td class="text-center">{{ $index + 1 }}</td>
                    <td class="text-center font-bold">{{ $so['code'] }}</td>
                    <td>{{ $so['name'] }}</td>
                    <td class="text-right">Rp {{ number_format($so['pagu'], 2, ',', '.') }}</td>
                    <td class="text-right">Rp {{ number_format($so['realisasi'], 2, ',', '.') }}</td>
                    <td class="text-center font-bold">{{ $so['pagu'] > 0 ? round(($so['realisasi'] / $so['pagu']) * 100, 2) : 0 }}%</td>
                </tr>
            @endforeach
